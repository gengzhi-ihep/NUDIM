...
int main(int argc, char *argv[]){
	...
	fftw_complex *slices, *tomograms, *data;
...
//				 fprintf(stderr,"Unhandled option '%c' \n", c);
	...
	tomograms = (fftw_complex*) nfft_malloc(sizeof(fftw_complex)*ntotal*nslices);
	...
	slices = (fftw_complex*) nfft_malloc(sizeof(fftw_complex)*ntotal1*nz);

	/* mask is used to label known slices */
	mask = (int*) malloc(sizeof(int)*ntotal1*nz);
	...
	for(j=0; j<nz; j++){
		...
		for(i=0; i<ntotal1; i++){
			...
			if( (angle < gam_min) || (angle > gam_max) || (seq%span != 0) ){
				...
				if(config_random){
					/*random padding*/
					slices[j*ntotal1+i] = gsl_rng_uniform(rng)*10;
				}else{
					/*zero padding*/
					slices[j*ntotal1+i] = 0;
				}

                        }else if( abs((int)angle) < 1e-5 ){
                                /* cross validation region */
                                mask[j*ntotal1+i] = 0;
				...
				slices[j*ntotal1+i] = tomograms[order*ntotal+j*np+col];
...
//			printf("Slice: %d Iteration: %d Residual: %f\n", j, i, ip.dot_r_iter);
		...
		for(i=0; i<p1.N_total; i++){
                    //ifftshift along axis Z after infft2 for moving zero-frequency to corner(used for ifft and consistent with ifft(ifftshift))
	            data[(j*np*np+i+np*np*nz/2)%(np*np*nz)] = ip.f_hat_iter[i];
		}
	...
	FILE *fp = fopen("recovered-0.txt","w");
	...
	for(nn=0; nn<nhio+ner; nn++){
		...
		for(j=0; j<nz; j++){
			...
			for(i=0; i<p1.N_total; i++)
				p1.f_hat[i] = data[(j*np*np+np*np*nz/2+i)%(np*np*nz)]; //fftshift
			...
			for(i=0; i<p1.M_total; i++){
				if(mask[j*p1.M_total+i] > 0.5){
					ip.y[i] = slices[j*p1.M_total+i];
					diff1 += pow(creal(p1.f[i]-slices[j*p1.M_total+i]),2)+pow(cimag(p1.f[i]-slices[j*p1.M_total+i]),2);
					diff2 += pow(creal(p1.f[i]+slices[j*p1.M_total+i]),2)+pow(cimag(p1.f[i]+slices[j*p1.M_total+i]),2);
				}else{
					ip.y[i] = p1.f[i];
				}
			}
			...
			for(i=0; i<p1.N_total; i++)
				...
				ip.f_hat_iter[i] = img[(j+nz/2)%nz][i/np][i%np];
			...
			//copy INFFT data for 1D IFFT
			for(i=0; i<p1.N_total; i++)
				data[(j*np*np+np*np*nz/2+i)%(nz*np*np)] = ip.f_hat_iter[i]; //ifftshift
